...
function mapp() {
    ...
    var buckets = 10,
        colors = ["#f7fbff", "#e1edf8", "#cadef0", "#abcfe6", "#82badb", "#59a1cf", "#3787c0", "#1c6aaf", "#0b4d94", "#08306b"];
    ...
    var allColors = {
        ...
        "active": ["#f7fbff", "#e1edf8", "#cadef0", "#abcfe6", "#82badb", "#59a1cf", "#3787c0", "#1c6aaf", "#0b4d94", "#08306b"],
        "recovered": ["#f7fcf5", "#e6f5e1", "#cdebc7", "#addea7", "#88cd87", "#5db96b", "#38a055", "#1b843f", "#04672b", "#00441b"],
    ...
    var map = div2.append("svg").attr("id", "#svg1").attr("width", 1200).attr("height", 1200).call(initialize);
    ...
    d3.json("data.json", function (error, d) {
        ...
        data = d1.map(function (d) {
            if (maxState.d.confirmed < d.confirmed)
                maxState.d = d; 
    ...
    function updateMap(data, grp) {
        maxTotal = d3.max(data, function (d) { return d.value });
      ...
      .enter().append('text')
        ...
        d3.select(this)
              ...
              .text("Highest value is in " + d.id + " (" + stateMap[d.id][grp] + ")");
        })
        .filter(function(d) {
          return d.id !== 'Maharashtra';
        }).remove();
    ...
    sel.on('change', function () {
        ...
        grp = selectedOption;
        data = d1.map(function (d) { return { "value": parseInt(d[grp]) }; })
        updateMap(data, grp)
    });
